...
class HIL : public StatObject {
 private:
  ...
  struct {
    uint64_t request[2]; // [NG] read, write
    uint64_t busy[3]; // [NG] read, write, total busy time
    uint64_t iosize[2]; // [NG] read, write
    uint64_t lastBusyAt[3]; // [NG] read, write, total busy time
  } stat;
  /*
   * Note, these stats only count those made by the host,
   * and does not include R/W caused by trim, flush, etc.
   */
